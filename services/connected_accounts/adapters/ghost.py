...
class GhostAdapter(BaseProviderAdapter):
    @classmethod
    ...
    def connect(self, user_id: int, **kwargs) -> Dict[str, Any]:
        ...
        if not token or not site_url:
            return {"ok": False, "error": "Missing token or site URL"}
        return {"ok": True, "token": token, "metadata": {"site_url": site_url}, "account_name": f"Ghost ({site_url})"}
    ...
    def publish(self, user_id: int, content: Any, preferences: Any = None) -> Dict[str, Any]:
        account = ConnectedAccount.query.filter_by(user_id=user_id, provider="ghost").first()
        if not account:
