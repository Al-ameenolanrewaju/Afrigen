...
class HashnodeAdapter(BaseProviderAdapter):
    @classmethod
    ...
    def connect(self, user_id: int, **kwargs) -> Dict[str, Any]:
        ...
        return {"ok": True, "token": token, "account_name": "Hashnode User"}
    ...
    def publish(self, user_id: int, content: Any, preferences: Any = None) -> Dict[str, Any]:
        account = ConnectedAccount.query.filter_by(user_id=user_id, provider="hashnode").first()
        if not account:
